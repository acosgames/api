...
function S3UploadGameBundles(req, res, cb) {
    return new Promise(async (resolve, reject) => {
        ...
        // busboy.on('')
        busboy.on("file", async function (fieldName, fileStream, info, encoding, mimeType) {
            let Bucket = "";
            let ContentType = "";
            let Key = "";
            let ACL = "";
            ...
            if (fieldName == "server") {
                ...
                const replaceStringStream = (str) => new Transform({
                    transform(chunk, enc, cb) {
                        let chunkStr = chunk.toString();
                        if (chunkStr.indexOf("//# sourceMappingURL=client.bundle.js.map") > -1) {
                            chunkStr = chunkStr.replace("//# sourceMappingURL=client.bundle.js.map", sourcemapString);
                            cb(null, chunkStr);
                        }
                        else {
                            cb(null, chunk);
            ...
            pass.on("data", (chunk) => {
                ...
                if (totalLength > MAXLENGTH) {
                    let errorMsg = "[ERROR] Exceeded upload limit: " +
                        (totalLength / 1000).toFixed(2) +
                        " kb";
                    pass.destroy(new Error(errorMsg));
...
//# sourceMappingURL=ACOSStorage.js.map