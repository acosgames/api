...
import { getCountry } from 'shared/services/country.js';
const JWT_PRIVATE_KEY = fs.readFileSync("./credential/jwtRS256.key", "utf-8");
export default class PersonAPI {
    ...
    async apiGetProfile(req, res, next) {
        try {
            ...
            if (!req.user.shortid)
                throw new GeneralError("E_NOTAUTHORIZED");
            let user = await persons.findUser({ shortid: req.user.shortid }, false);
            if (!user)
                throw new GeneralError("E_NOTAUTHORIZED");
            let filteredUser = {
                id: user.id,
    ...
    async apiCreateDisplayname(req, res, next) {
        ...
        try {
            ...
            if (user && user.displayname) {
                user.displayname = user.displayname.replace(/[^A-Za-z0-9\_]/gi, "");
                if (user.displayname.length < 3) {
                    res.json({ ecode: "E_DISPLAYNAME_TOOSHORT" });
                    return;
                }
            }
            let portraitid = user?.portraitid ||
