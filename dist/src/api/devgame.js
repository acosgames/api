...
import DevAuth from "./authentication/authdev.js";
const devauth = new DevAuth();
// const multer = require('multer')
import { S3UploadGameBundles, S3UploadGameImage, deletePreviews, } from "./ACOSStorage.js";
...
export default class DevGameAPI {
    ...
    routes(middleware) {
        ...
        this.router.get("/api/v1/dev/find/game/:game_slug", middleware, this.apiDevFindGame.bind(this));
        ...
        this.router.post("/api/v1/dev/createoredit/achievement", middleware, this.apiDevCreateOrUpdateAchievement.bind(this));
    ...
    async apiInviteGithub(req, res, next) {
        try {
            ...
            let dbUser = await db.sql(`SELECT * FROM person WHERE shortid = ?`, [user.shortid]);
            dbUser =
    ...
    async createOrUpdateGameVersion(apikey, hasDB, hasCSS, screentype, resow, resoh, screenwidth) {
        let game = { apikey };
        let gameFull = await devgame.findGame(game);
        let gameTest = await devgame.createGameVersion(gameFull, hasDB, hasCSS, screentype, resow, resoh, screenwidth);
        return gameTest;
    }
    ...
    async apiDevCreateOrUpdateAchievement(req, res, next) {
        ...
        try {
            ...
            let sessionUser = req.user;
            let pushedAchievement = await devgame.updateAchievement(game, achievement, sessionUser);
            if (!pushedAchievement) {
                throw new GeneralError("E_DEVGAME_CREATEACHIEVEMENTFAILED");
            }
            if (pushedAchievement.ecode)
                res.status(400);
